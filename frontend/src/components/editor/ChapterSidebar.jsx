...
const SortableItem = ({ chapter, index, selectedChapterIndex, onSelectChapter, onDeleteChapter, onGenerateChapterContent, isGenerating }) => {
    ...
    return (
        <div ref={setNodeRef}
            style={style}
            className="group relative flex items-center bg-white rounded-lg shadow-sm hover:shadow-md transition-shadow duration-200 relative overflow-hidden">
            <button
                ...
                <GripVertical
                    className="w-4 h-4 text-slate-400 mr-2 cursor-grab" {...listeners} {...attributes} />
                <span
                    className="truncate">{chapter.title}</span>
            </button>
            <div className="flex items-center ml-2 bg-white group-hover:opacity-100 transition-opacity px-2 py-3 absolute right-0">
                <Button
                    variant="ghost"
                    size="small"
                    className="py-2 px-2"
                ...
                <Button
                    ...
                    size="small"
                    className="py-2 px-2"
                    onClick={() => onDeleteChapter(index)} title="Delete Chapter">
...
const ChapterSidebar = ({
    ...
    return (
        <aside 
        className="w-80 h-full bg-white border-r border-slate-200 flex flex-col"
        >
